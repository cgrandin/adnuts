...
.sample_admb <- function(model,
  ...
  if(cores > cores.max) {
    cores <- cores.max - 1
    warning(paste0("Specified cores larger than available, using total - 1 = ", cores))
  }
  ...
  if(cores < 1){
    stop(paste0("Cores must be >= 1, but is ", cores),
         call. = FALSE)
  }
  parallel <- (cores == 1 || chains == 1)
  if(parallel && cores < chains){
    message(paste0("Recommend using chains < cores = ", cores))
  }
  stopifnot(thin >= 1)
  stopifnot(chains >= 1)
  if(is.null(seeds)){
  ...
  message(paste0("... Merging post-warmup chains into main folder: ", path))
